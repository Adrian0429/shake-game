...
const Page = () => {
  ...
  const handleDeleteTask = async (taskId: string) => {
    try {
      ...
      const response = await axios.delete(
        `https://api2.fingo.co.id/api/user/tasks/delete/${taskId}`, // Keep this as is for URL path
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
          // Send taskId as form data
          data: {
            task_id: taskId,
          },
        }
  ...
  return (
    <div className="flex flex-col h-screen w-screen p-5">
      <h1 className="text-2xl font-bold">News Tasks</h1>
      <Link
        ...
        {tasks.map((task) => (
          ...
          >
            <div>
              ...
              <p className="text-gray-500">{task.count}</p>
