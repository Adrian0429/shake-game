...
const OffCanvasPhone = ({ isVisible, onClose, onSuccess }: Props) => {
  ...
  const updateKYC = async (data: ClearRequest) => {
    try {
      ...
      if (response.data.status === true) {
        toast.success("successfully set phone number!");
        onSuccess();
        reset();
      }
    } catch (error) {
      toast.error("Error setting phone number");
  ...
  return (
    ...
    >
      ...
      <FormProvider {...methods}>
        ...
        >
          <MdPhoneInTalk className="text-[#CAEB45]" size={72} />

          <p className="text-white font-semibold text-2xl">
            Kirimkan Nomor Telepon Anda!
          </p>
